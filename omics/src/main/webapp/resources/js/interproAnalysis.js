...
//	ipr_color_type:["#0066b3", "#f598aa", "#70bf54", "#fec34e", "#ef403d", "#6850a1", "#934d2a", "#31bcad", "#21abcd", "#ffea82", "#ab3e97", "#eed903", "#71ceea", "#00854a", "#c02555", "#fab49b", "#a23e97", "#2d5980", "#f58220", "#b3beb1", "#008e83", "#f15b4e", "#ffd400", "#c48a10", "#5c6654", "#5c6654", "#ee3e75", "#00aebd", "#f99d1c", "#a6ce42", "#16568b"],
	...
	IPR_NMS : [],
	DomainType_NMS : ["Family", "Domain", "Active_site", "Binding_site", "Conserved_site", "PTM", "Repeat", "Unintegrated"],
	protein_name:'',
	item_bar_height:0,
	gap:0,
	width:0,
	...
	getIprDomainTypeColor:function(type) {
		if( type=="Family") 				return "red";
		else if( type == "Domain" )			return "green";
		else if( type == "Active_site" ) 	return "blue";
		else if( type == "Binding_site" ) 	return "purple";
		else if( type == "Conserved_site" )	return "orange";
		else if( type == "PTM" )			return "brown";
		else if( type == "Repeat")			return "pink";
		else								return "gray";
	},
	init:function( obj ) {
		this.margin					= 8;
		...
		this.gap					= 5;
	 	this.protein_name			= obj.protein_id;	 	
...
//	 	this.resize( $("#gene-structure").width() );
		this.resize( 780 );
	 	...
	 	var jsonQuery = JSON.stringify( obj );
 	 	...
 	 	this.initIpr( jQuery.parseJSON(jsonQuery) );
  	 	...
  	 	this.initIprTracks( jQuery.parseJSON(jsonQuery) );
	},
	...
	initIprTracks:function(obj) {
		var noIPR = 'no IPR';
		
		for(var i=0; i<obj.dbs.length; i++) {//IPR set 만들기( 예를들어 DB set은 위에 배열로 박아뒀다. 하지만 IPR은 종류가 많아서 박아둘수가없다. 때문에 set을 만들어야한다 )
			if(isEmpty(obj.dbs[i].ipr_id))
				obj.dbs[i].ipr_id = noIPR;
			
			if($.inArray(obj.dbs[i].ipr_id, this.IPR_NMS) == -1)// 배열 내에 this.IPR_NMS 값이 없는 경우
				this.IPR_NMS.push(obj.dbs[i].ipr_id);
		}
		
		if($.inArray(noIPR, this.IPR_NMS) != -1) {
			this.IPR_NMS = $.grep(this.IPR_NMS, function(value) {
				return value != noIPR;
			});
			
			this.IPR_NMS.push(noIPR);
		}
		
		console.log(this.IPR_NMS);
		
		for(var i=0; i<this.IPR_NMS.length; i++) {
			var arr = [];
			for(var j=0; j<obj.dbs.length; j++) {
				if( this.IPR_NMS[i] == obj.dbs[j].ipr_id ) {
					obj.dbs[j].layer_no = -1;
 					arr.push( JSON.stringify(obj.dbs[j]) );
				}
			}
			...
			if( arr.length > 0 ) {
				var ipr_str = '{"ipr":"' + this.IPR_NMS[i] + '", "domains":[';

				for(var j=0; j<arr.length; j++) {
					ipr_str += arr[j];
					if( j != arr.length-1 )	ipr_str += ',';
				}

				ipr_str += ']}';

				var jsonObj = jQuery.parseJSON( ipr_str );

				jsonObj.domains = this.getComputeDepthOfLayer( jsonObj.domains );

  				this.ipr_tracks.push( jsonObj );
			}
		}
	},
...
//		console.log( this.ipr_terms );
	...
	paint:function( obj ) {
		var base = $("#analysisViewerPanel").css("width", this.width);

		var totalHeight = this.drawProteinWithIprTerm( base );
		
		for(var i=0; i<this.IPR_NMS.length; i++) {
			for(var j=0; j<this.ipr_tracks.length; j++) {
				if( this.IPR_NMS[i] == this.ipr_tracks[j].ipr && this.ipr_tracks[j].domains.length > 0 ) {
					console.log(this.ipr_tracks[j]);
					
					if(isEmpty(this.ipr_tracks[j].domains[0].type))
						this.ipr_tracks[j].domains[0].type = '?';
					
					var domainTrackHeight = this.addDomainTracks( base, this.IPR_NMS[i], this.ipr_tracks[j].domains, this.ipr_tracks[j].domains[0].type );
		...
		for(var i=0; i<this.DomainType_NMS.length; i++) {
			var iprDomainTypeLegend = "";
			
			if(i != this.DomainType_NMS.length-1) {
				iprDomainTypeLegend = $("<div class='iprDomainTypeColor' style='width:15px; height:15px; margin-left:15px; margin-right:5px; text-align:center; float:left; border-radius:5px;" +
						"border:1px solid #d3d3d3; background:"+this.getIprDomainTypeColor(this.DomainType_NMS[i])+"'>" +
								"<p class='iprDomainType'>"+this.DomainType_NMS[i].substring(0,1)+"</p></div><div>"+this.DomainType_NMS[i]+"</div>");
			} else {
				iprDomainTypeLegend = $("<div class='iprDomainTypeColor' style='width:15px; height:15px; margin-left:15px; margin-right:5px; text-align:center; float:left; border-radius:5px;" +
						"border:1px solid #d3d3d3; background:"+this.getIprDomainTypeColor(this.DomainType_NMS[i])+"'>" +
								"<p class='iprDomainType'>"+"?"+"</p></div><div>"+this.DomainType_NMS[i]+"</div>");
			}
			
			$('#viewerPanelFieldsetLegend').append(iprDomainTypeLegend);
		}
...
//		}
		...
		var cntDrawingDb = $("#analysisViewerPanel").children().length;

		totalHeight += ( cntDrawingDb * parseInt($(".track").css("margin-top")) );
		totalHeight += $("#analysisViewerPanel > .anlysisTitle").height();
		
//		base.css("height", totalHeight);
		
		console.log("asd : " + totalHeight);
...
*/
	...
	addDomainTracks:function( base, DB_NM, db_item, type ) {
		var domainTrack = $("<div id='"+DB_NM+"-layer' class='track'></div>");
		var domainTitle = $("<div id='"+DB_NM+"-name' class='track-name'><span class='track-name-label-header'></span><span class='domain-track-name-lable'>"+DB_NM+"</span></div>");
		var domainPanel = $("<div class='viewerFrame'></div>");
		
		var iprDomainTypeColor = $("<div class='iprDomainTypeColor' style='width:15px; height:15px; margin:11px 0 0 15px;text-align:center;float:left; border-radius:5px;" +
				"border:1px solid #d3d3d3; background:"+this.getIprDomainTypeColor(type)+"'><p class='iprDomainType'>"+type.substring(0,1)+"</p></div>");
		
		domainTrack.append( iprDomainTypeColor );
...
function getMessageFromServer() {
	...
	var urlSplit = url2.split("?");
	
	url2 = urlSplit[0] + "/../" + "getInterproAnalysis";
	
//	console.log(p_tax_id + " " + p_refseq_assembly_id + " " + p_protein_accession_id);
...
//	var tax_id = '210';
//	var refseq_assembly_id = 'GCF_000688915.1';
//	var protein_accession_id = 'WP_026938321.1';
	
//	var tax_id = "1257118";
//	var refseq_assembly_id = "GCF_000313135.1";
//	var protein_accession_id = "XP_004333665.1";
	
//	var tax_id = "500485";
//	var refseq_assembly_id = "GCF_000226395.1";
//	var protein_accession_id = "XP_002557265.1";

//	var tax_id = "10029";
//	var refseq_assembly_id = "GCF_000223135.1";
//	var protein_accession_id = "XP_007649008.1";
	
//	var tax_id = "2711";
//	var refseq_assembly_id = "GCF_000317415.1";
//	var protein_accession_id = "XP_006464290.1";
	
//	var tax_id = "549";
//	var refseq_assembly_id = "GCF_000710215.1";
//	var protein_accession_id = "WP_024979301.1";
	
//	var tax_id = "4081";
//	var refseq_assembly_id = "KOBIC_2";
//	var protein_accession_id = "Solyc00g007110.2.1";
	
//	var tax_id = "3329";
//	var refseq_assembly_id = "KOBIC_1";
//	var protein_accession_id = "MA_1001810g0020";
	
//	var tax_id = "1727";
//	var refseq_assembly_id = "GCF_000720035.1";
//	var protein_accession_id = "WP_030199350.1";
	
//  	var ajaxRetObj = getMessageFromServer( tax_id, refseq_assembly_id, protein_accession_id );
